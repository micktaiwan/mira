...
function readCookieRows(cookiesDbPath) {
  ...
  const query =
    "select host_key, name, path, expires_utc, is_secure, is_httponly, samesite, hex(encrypted_value), coalesce(value,'') from cookies"
  const out = node_child_process.execFileSync(
    'sqlite3',
    [tmp, '-newline', RECORD, '-separator', FIELD, query],
    {
      encoding: 'utf8',
...
const DEFAULT_CHROME_DIR = node_path.join(
  os.homedir(),
  'Library',
  'Application Support',
  'Google',
...
const STOP_ACTIONS = ['clearSelection', 'keepSelection', 'activateSelection']
const findCommands = {
...
const BUILTIN_SKILLS = [
  {
    ...
    match: {},
    prompt:
      'Summarize the following page content in a few clear, concise bullet points. Keep only what matters; drop navigation and boilerplate.',
...
const CHAT_SYSTEM_PROMPT =
  "You are a helpful assistant embedded in a web browser. Answer the user's questions clearly and concisely. Use the current page (its URL and its text) as the context for the conversation."
const CHAT_WEB_ONLY_PROMPT =
  'You are a chat assistant embedded in a web browser. You have exactly two tools: WebSearch (to search the web) and WebFetch (to read a URL). Use them when a question needs current or external information beyond the conversation and the provided page context. You have NO other tools: no shell, no filesystem, and no ability to control the browser. If asked to do something outside answering and web lookups, say so plainly in one sentence.'
...
const skillsCommands = {
  ...
  'run-prompt': async (ctx, params) => {
    ...
    try {
      ...
      const answer = await ctx.chat(withUser, {
        url: url2,
        text,
...
const KINDS = /* @__PURE__ */ new Set(['image', 'video', 'audio', 'svg', 'canvas', 'font', 'other'])
function parseDomMedia(raw) {
  ...
  for (const entry of arr) {
    ...
    const kind = hasMime
      ? classifyMedia({ mime: entry.mime, url: url2 })
      : typeof entry.kind === 'string' && KINDS.has(entry.kind)
        ? entry.kind
        : classifyMedia({ url: url2 })
...
async function encrypt(plan, password) {
  ...
  try {
    ...
    for (const dir of plan.dirs) {
      if (fs.existsSync(dir.live))
        fs.cpSync(dir.live, path.join(mountPoint, dir.name), { recursive: true })
    }
    for (const dir of plan.dirs) {
...
function normalizeWindow(value) {
  ...
  const v = value
  const windowId =
    typeof v.windowId === 'string' && v.windowId !== '' ? v.windowId : node_crypto.randomUUID()
...
function interpretRuntimeEvaluate(reply) {
  ...
  if (ex) {
    const message =
      ...
      (typeof ex.exception?.value === 'string' ? ex.exception.value : void 0) ??
      ex.text ??
      'evaluation failed'
...
function normalizePickRequest(payload) {
  const p = payload ?? {}
  const devices = (raw, kind) =>
    Array.isArray(raw)
      ? raw
          .map((d) => d)
          .filter((d) => d && typeof d.deviceId === 'string' && d.deviceId)
          .map((d) => ({ deviceId: d.deviceId, label: String(d.label ?? ''), kind }))
      : []
  return {
...
function loadAddon$1() {
  ...
  const candidate = electron.app.isPackaged
    ? path.join(process.resourcesPath, 'mira_spaces.node')
    : path.join(__dirname, '../../native/mira-spaces/build/Release/mira_spaces.node')
  try {
...
function loadAddon() {
  ...
  if (process.platform !== 'darwin') return null
  const candidate = electron.app.isPackaged
    ? path.join(process.resourcesPath, 'mira_location.node')
    : path.join(__dirname, '../../native/mira-location/build/Release/mira_location.node')
  try {
...
class ProfileManager {
  ...
  openUrl(url2, profileId) {
    ...
    if (profileId) {
      ...
      target =
        this.findByWindow(electron.BrowserWindow.getFocusedWindow()) ??
  ...
  trackDownload(item, profileId) {
    ...
    let name = suggested
    for (let i = 1; node_fs.existsSync(node_path.join(dir, name)); i++)
      name = numberedFilename(suggested, i)
  ...
  openLocationSettings() {
    ...
    electron.shell
      .openExternal(url2)
      .catch((error) => console.error('[mira] open location settings', error))
    return { opened: true }
  }
  ...
  currentBounds(pw) {
    ...
    const location =
      wid === void 0 ? void 0 : windowSpaceLocation(spacesLayout(), windowSpaces(wid))
  ...
  attachTab(src, dst, tabId, insertion) {
    ...
    if (insertion && !wasPinned) {
      ...
      if (over && over.id !== tabId) {
        ...
        dst.state = moveTab(dst.state, tabId, from < insertBefore ? insertBefore - 1 : insertBefore)
      }
    }
  ...
  async inspectCookiesInActive(pw) {
    ...
    if (created && host.webContents.isLoadingMainFrame()) {
      await new Promise((resolve) => host.webContents.once('did-finish-load', () => resolve()))
    }
    try {
  ...
  makeContext(target) {
    ...
    return {
      ...
      openDownload: async (id) => {
        ...
        if (!record || record.state !== 'completed' || !node_fs.existsSync(record.savePath))
          return false
...
const RELAXED_POLICY_FEATURES = ['camera', 'microphone', 'display-capture']
...
class ExtensionCaptureService {
  ...
  installIpc() {
    if (this.ipcInstalled) return
    this.ipcInstalled = true
    electron.ipcMain.handle(CHOOSE_DESKTOP_MEDIA_IPC_CHANNEL, async (event, payload) => {
...
function parseCommandShortcut(shortcut, platform) {
  ...
  for (const rawPart of shortcut.split('+')) {
    ...
    else if (lower2 === 'search') return null
    else if (lower2.startsWith('media')) return null
  ...
  attach(ses) {
    ...
    ses.serviceWorkers.on('running-status-changed', ({ versionId }) => {
      ...
      worker.ipc.handle(OFFSCREEN_IPC_CHANNEL, (_event, payload) =>
        this.handle(ses, extensionId, payload)
      )
    })
...
class ExtensionsService {
  constructor(deps) {
    ...
    electronChromeExtensions.setSessionPartitionResolver((partition) =>
      ...
      submenu: [
        ...
        ...(() => {
          ...
          return tree.length
            ...
            : [{ label: 'No favorites', enabled: false }]
        })()
      ]
    },
    {
...
electron.app.whenReady().then(async () => {
  ...
  electron.app.userAgentFallback = electron.app.userAgentFallback.replace(
    / (?:Mira|Electron)\/\S+/g,
    ''
  )
  installStealth()
  ...
  const loadProfilePermissions = (id) =>
    loadProfileJson(id, 'permissions.json', normalizePermissions)
  const persistProfilePermissions = (id, permissions) =>
  ...
  const persistProfileBookmarks = (id, bookmarks) =>
  ...
  const extensionsService = new ExtensionsService({
    ...
    extensionsDirFor: (profileId) =>
      path.join(electron.app.getPath('userData'), 'Extensions', profileId)
  })
  ...
  const profiles = new ProfileManager({
    ...
    getProcessMemory: () =>
      electron.app
        .getAppMetrics()
        .map((m) => ({ pid: m.pid, bytes: m.memory.workingSetSize * 1024 })),
