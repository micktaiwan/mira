...
3. Le profil sort dans `<dd>/Build/Products/Debug/<App>.app/Contents/embedded.provisionprofile` — Xcode 26 n'écrit **rien** dans `~/Library/Developer/Xcode/UserData/Provisioning Profiles/`. Le copier sur `build/embedded.provisionprofile`, puis `./bin/build.sh`.
4. Vérifier : `security cms -D -i build/embedded.provisionprofile | plutil -extract ExpirationDate raw -` doit donner J+7, et les entitlements doivent contenir `keychain-access-groups = ZMKDR6H89Y.*`. Après rebuild, contrôler que les helpers sont vivants (`ps -Ao comm | grep -c "[M]ira Helper"` — un SIGKILL AMFI les tuerait) et que `isUserVerifyingPlatformAuthenticatorAvailable()` renvoie `true` dans une page.
