...
**Electron ≥ 42 ne télécharge plus son binaire au `npm install`.** Après avoir changé la version d'`electron`, lancer `npx install-electron --no` avec un Node ≥ 22.12 (`engines` du paquet ; le Node 22.11 par défaut du shell plante en `ERR_REQUIRE_ESM` sur `@electron/get`) : `PATH=~/.nvm/versions/node/v22.23.0/bin:$PATH npx install-electron --no`. Sans ça `node_modules/electron/dist` n'existe pas et `npm run dev` n'a rien à lancer. Passé de 41.7.0 à 44.3.0 le 2026-09-15 pour le fix upstream electron/electron#53257 (use-after-free de `pending_processes_` à la relance d'un renderer partagé, qui faisait planter le main quand le service worker d'une extension crashait).
