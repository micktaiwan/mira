...
describe.skipIf(!ENABLED)('vault-service (real hdiutil)', () => {
  ...
  it('encrypt: builds the vault and wipes the plaintext', async () => {
  ...
  it('unlock: restores the plaintext from the vault, intact', async () => {
  ...
  it('lock then unlock: a change made while unlocked persists into the vault', async () => {
    ...
    await unlock(plan, PASSWORD)
    expect(readFileSync(join(plan.dirs[0].live, 'sample.txt'), 'utf8')).toBe(
      'edited while unlocked'
    )
  }, 180_000)

  it('unlock with the wrong password is rejected', async () => {
