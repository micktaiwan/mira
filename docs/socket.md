...
```bash
bin/mira tabs                                # list tabs, * = active, z = asleep, ♪ = sound, + 3 age columns
...
bin/mira reload                              # reload the pinned tab (server-side) or the active one
...
**No command reached over this socket (or over MCP) brings Mira to the foreground.**
`new-tab`, `navigate`, `open-url`, `open-profile`, `activate-tab`, `press-key`,
`detach-tab`, `move-tab-to-window` all do their work in place: tabs open, pages
load, windows are created — but the app never activates, so whatever the user is
typing in keeps the keyboard. The one exception is `focus-app`, whose entire job
is to come forward — so it is the only door left, and an agent must not walk
through it to make its own work easier: Mira sits open behind the user's editor
all day, and every raise cuts what they were typing. Call it only when they asked
...
context the transport builds (`contextForChrome` = ui, `contextForFocused` =
external, in `src/main/profiles.ts`).

This is the deliberate half of the story. The _involuntary_ half — Chromium
activating the app by itself when a background page reloads — is handled by the
native swizzle in `src/main/mac-activation.ts`.

Note the two different things `background:true` means on `new-tab` / `navigate`:
not foregrounding the APP is now automatic, while `background` additionally keeps
the window from switching onto the new tab, so nothing changes on screen at all.
...
- Tab ids are **UUIDs, globally unique across all windows, and stable across a restart**
  ...
  fresh one). `exec-js`, `close-tab` and `discard-tab` resolve their
...
- `list-tabs` defaults to the target (focused) window, but takes an optional
  ...
  is discoverable without focusing each window first. An unknown id fails with
  `unknown window: <id>` rather than falling back to the focused window.
- `navigate` takes the same optional `tabId`: with it, the url is loaded into THAT
  tab in whatever window it lives (and with `newTab`, the new tab opens beside it, so
  the target picks the window). Without it, the command still hits the active tab of
  the focused window — which is why an external caller should always pass one. An
  unknown id fails with `unknown tab: <id>`; nothing is ever loaded into a fallback
  tab. This closed a real accident: three tabs of a working window were silently
  overwritten by navigations aimed at another window's tab (2026-08-28).
- `focus-app` takes an optional `windowId` (from `list-windows`) naming WHICH window
  comes forward. Without it Mira raises its last-focused window, so with several open
  the app kept coming back on the same one whatever the caller meant.
...
Every tab in `list-tabs` carries three timestamps, epoch ms, so a caller can tell
what has been sitting there for weeks without being read — the input for closing
stale tabs. They are persisted with the session, so they survive a restart and a
tear-off into another window; a tab is the same tab, with the same age.

- `openedAt` — when the tab was opened. `null` only for a tab restored from a
  session file written before this existed.
- `lastActiveAt` — when the tab was last made active (a selection, or a neighbor
  inheriting focus when its neighbor closed). `null` when it was never focused: a
  tab opened in the background, or restored asleep and never opened since. A tab
  that has been active for hours keeps the stamp of the moment it took focus.
- `updatedAt` — when the PAGE last changed (navigation, title, favicon). Not
  touched by bookkeeping (pinning, moving between folders), so it measures the
  page and not the tab. `null` when nothing changed since it was opened.

`bin/mira tabs` renders the three as compact ages (`3d`, `4h`, `-` when null) in
that order, between the tab id and the title.
...
| Command                               | Params                                    | Effect / result                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `navigate`                            | `url`, `tabId?`, `newTab?`, `background?` | load a (normalized) url; `newTab:true` opens a new tab, `background:true` (with `newTab`) loads it hidden without switching to it. `tabId` names the tab to load (any window) — with `newTab`, the tab the new one opens NEXT TO. An explicit `tabId` skips the same-url dedup (a named target is loaded, never swapped for a twin) and refuses an internal `chrome://` page rather than ignoring the target. Errors: `unknown tab: <id>` |
| `focus-address-bar`                   | —                                         | put keyboard focus on the address bar with its contents selected (Cmd+L, menu File → Open Location…). On the error page the bar holds the URL that FAILED, so Enter re-navigates to it. Also copies the active url + "Copied!" toast (`{copied}` when it did)                                                                                                                                                                                                                                                    |
| `back` / `forward`                    | —                                         | session-history step                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `reload` / `hard-reload`              | `tabId?`                                  | reload the named tab (any window), else the active one; `hard-reload` bypasses the HTTP cache. On Mira's error page it RETRIES the url whose load failed instead of re-rendering the error (that page is a data: URL, so a plain reload could never get off it) — the reply then carries `retried:true`. Errors: `unknown tab: <id>`                                                                                                      |
...
| Command                                  | Params                | Effect / result                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list-tabs`                              | `windowId?`           | `{tabs, activeId, panelCollapsed}` — tabs of the target window, with their UUID ids. Each tab carries `openedAt` / `lastActiveAt` / `updatedAt` (epoch ms, or null — see **Tab ages** below). With `windowId` (from `list-windows`), lists that window instead and echoes it back; errors: `unknown window: <id>`                                                                                                                                                         |
| `new-tab`                                | `url?`, `background?` | open a tab (default: home). `background:true` opens it hidden: the window does not even switch to it, so nothing changes on screen. (A socket `new-tab` never foregrounds Mira either way — see **Foreground** above.)                                                                                                                                                                                                                                                    |
...
| `activate-tab`                           | `id`                  | make a tab the VISIBLE/active tab in its own window, wherever it lives — the cross-window counterpart to `select-tab`. Does NOT raise the window or activate the app (see **Foreground** above); pair with `focus-app` if you really want Mira in front. Needed before `press-key`: Chromium only delivers input to a visible tab. `{windowId, id}`. Errors: `unknown tab`                                                                                                |
| `copy-active-url`                        | —                     | copy the active tab's url to the OS clipboard and flash a "Copied!" toast. Also what Cmd+L (`focus-address-bar`) fires. `{url}`. Errors: `no url to copy` (nothing active, or no url yet)                                                                                                                                                                                                                                                                                    |
| `close-tab`                              | `id`                  | close a tab                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `close-active-tab`                       | `focus?`              | close the active tab (Cmd+W semantics, pinned tabs are guarded). `focus` picks the tab that inherits: `neighbor` (default, the strip neighbor) or `recent` (the last tab viewed, Cmd+Alt+Shift+W)                                                                                                                                                                                                                                                                         |
...
| `sleep-all-tabs`                         | —                     | put every loaded tab to sleep except the pinned, keep-awake and active ones (File menu "Sleep All Except Pinned"); focus does not move. Returns `slept`                                                                                                                                                                                                                                                                                                                  |
| `prev-tab` / `next-tab`                  | —                     | cycle the strip (Cmd+Up / Cmd+Down)                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `recent-tab-back` / `recent-tab-forward` | —                     | back / forward through recently-viewed tabs (focus history, not strip order), deduplicated, per window — Cmd+Shift+Left / Cmd+Shift+Right. Returns `{id}` of the tab landed on, or `id:null` at a history end                                                                                                                                                                                                                                                             |
...
| `group-duplicate-tabs`                   | —                     | tidy the strip: exact duplicates (normalized url) gathered under their first occurrence, then same-site tabs brought together — grouped by registrable domain, then by host. Pinned tabs and tabs inside a folder never move; nothing is opened, closed or focused. Returns `moved` (0 = already tidy)                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `set-window-fullscreen`                  | `fullScreen?`, `windowId?` | enter / leave native macOS fullscreen; omit `fullScreen` to toggle. Targets the context's window unless `windowId` (from `list-windows`) is given. `{windowId, fullScreen}` — the state it ENDED in. The way out of a window stuck fullscreen on its own Space |
| `set-window-maximized`                   | `maximized?`, `windowId?` | maximize (zoom) or restore the window; omit `maximized` to toggle. A fullscreen window is left alone (leave fullscreen first). `{windowId, maximized}` |
| `list-windows`                           | —                     | `{windows}` — every open window: `{windowId, profileId, tabCount, bounds, focused, fullScreen, maximized}`. `fullScreen`/`maximized` are the geometry MODE (a window merely sized to the display is neither). A profile can have several windows (a tear-off)                                                                                                                                                                                                                                                                                                                                      |
| `close-window`                           | `windowId?`           | close ONE window. Without `windowId`, the window the user means: the focused one, else the last that held focus — never an arbitrary one. `{windowId, closed}`. From the socket it is tagged script-closing, so it never quits Mira even as the last window (use `quit` for that); the same command from the Cmd+Shift+W menu item is a user close and does quit, through the confirmation. Errors: `unknown window: <id>`, `no target window`                            |
...
| `press-key`       | `key`, `tabId?`, `modifiers?`             | send a REAL keypress (CDP `Input.dispatchKeyEvent`, `isTrusted:true`) to a tab — for keyboard-driven web apps (archive with `e`, `j`/`k`, `Escape`) that ignore synthetic DOM events. `key` is a `KeyboardEvent.key` name (`e`, `Enter`, `ArrowDown`, ` `); `modifiers` ⊆ `alt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ctrl    | meta     | shift`. The tab is **made the visible tab of its window first** (and its window put back on screen if minimized/covered, still without activating the app) so a hidden tab does not silently drop the key. Result `{key}`. Errors: `unknown tab`, `tab could not be made visible for input (Mira may be hidden — \`focus-app\` first)` |
| `click`           | one of `selector` / `text` / `x`+`y`, plus `tabId?`, `nth?`, `scroll?`, `modifiers?` | a REAL mouse click (CDP `Input.dispatchMouseEvent`: move, press, release), the counterpart of `press-key` for the pointer. A `selector` or `text` target is resolved INSIDE the page to the centre of its box; `x`/`y` are raw viewport coordinates. `text` keeps the DEEPEST element carrying it (every ancestor up to `<body>` contains the words). `nth` (1-based) picks among several visible matches; `scroll:true` scrolls the target into view first. Same visibility guard as `press-key`. Result `{x, y, target}`. Errors: `no match`, `found N match(es), none visible`, `asked for match N of M`, `the target is outside the viewport …; pass scroll:true`, `the target is covered by <tag> at that point`, `unknown tab`, `tab could not be made visible for input` |
| `wait-for`        | one of `selector` / `text` / `url`, plus `tabId?`, `gone?`, `timeoutMs?` | block until the condition holds in the page, so a script never has to `sleep`. `selector` also requires a non-zero box (an element mid-animation is not there yet), `text` matches rendered `innerText`, `url` a substring of `location.href`; `gone:true` waits for the condition to STOP holding. `timeoutMs` defaults to 5000, capped at 300000. Polled in-page every 100 ms as many short evaluations, never one long await (exec-js is capped at 5 s). Result `{waitedMs, condition}` — the time it really took, not the budget. Errors: `timed out after <n>ms waiting for <condition>`, `unknown tab: <id>`, `no active web page` |
| `get-console`     | `tabId?`, `level?`, `limit?`, `sinceSeq?` | read a tab's captured web-page console (`messages[]`). Tails console.\* calls AND browser-emitted lines (failed loads/403, CORS, CSP, uncaught exceptions) into a per-tab ring buffer — the thing you'd otherwise need DevTools open to see. Covers the page AND its cross-origin iframes and workers (CDP auto-attach), so what a payment or login widget logs from inside its own frame is captured too — read `url` to tell which frame a line came from. `level` floors severity (`verbose` \| `info` \| `warning` \| `error`), `limit` caps to the most recent N, `sinceSeq` returns only entries newer than a seq (polling). Without `tabId`, the active tab. Survives a discard (asleep tab keeps its history); dropped on real close. Each entry: `{ seq, level, message, source, url?, lineNumber? }` where `source` ∈ `console | network | security | exception                                                                                                                                                                                                                                                                                                                              | other`. Errors: `unknown tab: <id>`, `no active web page` |
...
| `analyze-page-audio` | `tabId?` | Manual, one-shot audio discovery. Returns `{media, count, unavailable}`. Verifies current audio blobs (32 MB total read budget, up to 20 DOM sources) and includes known network audio. Readable blobs carry `audioDownloadable: true` and `audioTabId`. Does not save files, start playback or install capture hooks. MediaSource/expired/oversized blobs remain unavailable. |
| `download-page-audio` | `url`, `tabId?` | Explicitly save a verified blob audio source from its original tab to Downloads. Returns `{saved: 1}`. Revalidates that the audio element still uses this URL; errors if changed, expired, unreadable or over 32 MB. Use the analysis item's `audioTabId`. |
...
### Downloads (browser file downloads)

Every file a page triggers is saved straight to `~/Downloads` (no OS save dialog — a colliding name gets a ` (n)` suffix) and tracked here, distinct from the media-gallery grabs above. The status bar shows progress and flashes a toast on completion; `mira:downloads-changed` is pushed to the chrome on every change.

| Command              | Params | Effect / result                                                                                                                                                                                                                 |
| -------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list-downloads`     | —      | every tracked download, newest first: `{downloads: DownloadRecord[], count}`. `DownloadRecord = {id, url, filename, savePath, state, receivedBytes, totalBytes, paused, startedAt, updatedAt, profileId}`; `state ∈ progressing | completed | cancelled | interrupted` |
| `cancel-download`    | `id`   | cancel a running download; error `no active download: <id>` when unknown/finished                                                                                                                                               |
| `open-download`      | `id`   | open a completed download with the OS default app; error when unknown, unfinished, or the file is gone                                                                                                                          |
...
| Command                | Params          | Effect / result                                                                                                                                                                                                                                                                  |
| ---------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list-profiles`        | —               | profiles + which are open + focused id + each profile's `origin` (`user` \| `automation`)                                                                                                                                                                                                                                           |
| `open-profile`         | `id`            | open a profile window (creating it if closed, shown without activating the app), and make it the target of later commands. Over the socket it does not focus it — see **Foreground** above                                                                                       |
...
| `create-profile`       | `label?`        | new profile. Created over the socket/MCP it is tagged `origin: automation` — and that is the ONLY kind `delete-profile` may remove from here                                                                                                                                                                                                                                                                      |
| `rename-profile`       | `id`, `label`   | rename                                                                                                                                                                                                                                                                           |
| `delete-profile`       | `id`            | delete a profile FOR GOOD: its saved tabs, cookies/storage partition, history, favorites and (if encrypted) its vault. Irreversible. **Over the socket/MCP it only accepts a profile automation created** (`origin: automation`); a profile made by hand is deleted by the user, in Settings. Errors: `unknown profile: <id>`, `the default profile cannot be deleted`, `profile <id> was not created by automation: delete it from Settings`, `close the profile window before deleting it` |
...
A theme paints a profile's chrome (toolbar, sidebar, palette, new-tab/error pages). A theme is just a few colors, from which the whole palette is derived: `background`, `text`, optional `accent` (all `#rgb`/`#rrggbb`) and an optional `wallpaper` (an http(s) image URL painted behind the chrome). Built-in themes (`midnight`, `slate`, `paper`, `sepia`) are always present and cannot be edited/deleted; custom themes are persisted to `userData/themes.json`. Assign one to a profile with `set-profile-theme` (above).

| Command        | Params                                                     | Effect / result                                                               |
| -------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `list-themes`  | —                                                          | every theme (built-ins first, then custom): `{themes:[…]}`                    |
| `create-theme` | `name`, `background`, `text`, `accent?`, `wallpaper?`      | create a custom theme; returns `{theme}`. Id is derived from the name         |
| `update-theme` | `id`, `name?`/`background?`/`text?`/`accent?`/`wallpaper?` | patch a custom theme (accent/wallpaper `null` clears). Built-ins refuse       |
| `delete-theme` | `id`                                                       | delete a custom theme (built-ins refuse). Profiles on it fall back to default |

Example: `mira call create-theme --params '{"name":"Ocean","background":"#0d1b2a","text":"#f0e6d2","accent":"#39a0ed"}'` then `mira call set-profile-theme --params '{"id":"<profile>","themeId":"ocean"}'`.
...
| `save-login`       | `url`, `password`, `username?`, `profileId?`       | write a login straight into the vault. `{id, label, updated, linked}` — `updated:true` means the account was already there and its password was replaced instead of duplicated; `linked:true` means this exact credential was already saved on another subdomain of the same site, and only that address was added to it. Refuses a non-http(s) url or a password under 4 characters                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `login-candidates` | `tabId?`, `profileId?`                             | which stored accounts could log in on that tab's page: `{profileId, url, host, candidates:[{id,name,username,hosts,exact}]}` — never a password, and nothing is typed into the page. `exact:true` marks the accounts filed under this very host, as opposed to a sibling subdomain of the same site. A locked vault pops the native master-password bubble instead of failing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `fill-login`       | `tabId?`, `profileId?`, `id?`, `username?`, `ask?` | put a credential the vault holds into that tab's login form. Answers `{filled:{id,name,username,username_filled,password_filled,frames,passwordFields}}` when it filled, or `{candidates:[…]}` when it did not — and then **nothing was typed**. With no `id`/`username`, Mira fills straight away when the answer is not a guess (one matching account, or the one chosen last time on that site, remembered per profile in `userData/login-fill.json`); when several accounts match it opens its **native picker** in that profile's window and fills what is chosen there. `ask:false` skips the window and answers with the list instead — for a caller that must not block on a human. Never returns the password, never submits the form. The profile is the one that OWNS the tab, not the focused window's. Also bound to **Cmd+Shift+L** (Edit → Fill Login from Vault), which runs it on the focused window's active tab and toasts what it could not do. Errors: `no card vault for profile: <id>`, `not a web page — nothing to fill here`, `no login saved for <host>`, `unknown tab: <id>` |
...
| `clear-site-data`      | `url?`, `reload?`                                         | cookies + storage for one site (default: active site); `reload: true` reloads the target tab right after, so the page comes back signed out (Cmd+Alt+Backspace)                                                                                                                                                                                                                                                                                |
